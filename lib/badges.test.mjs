...
import { BADGE_IDS, formatBadgeRow, parseFlags, renderBadge } from "./badges.mjs";
...
describe("renderBadge", () => {
  it("renders CI badge", () => {
    ...
    assert.match(badge.href, /acme\/demo\/actions/);
  });

  it("renders npm badge", () => {
    const badge = renderBadge({ npm: "@acme/pkg" }, "npm");
    assert.match(badge.src, /npm\/v\/@acme\/pkg\.svg/);
    assert.match(badge.href, /npmjs\.com/);
  });

  it("requires npm for npm badge", () => {
    assert.throws(() => renderBadge({}, "npm"));
  });
  ...
  it("renders stars badge", () => {
    ...
    assert.match(badge.href, /github\.com\/acme\/demo/);
  });
  ...
  it("renders default license badge", () => {
    const badge = renderBadge({}, "license");
    assert.match(badge.src, /License-MIT/);
  });
  ...
  it("renders default node badge", () => {
    const badge = renderBadge({}, "node");
    assert.match(badge.src, /node-%3E%3D20/);
  });
  ...
  it("renders release badge", () => {
    ...
    assert.match(badge.href, /releases/);
  });

  it("exports badge ids", () => {
    assert.ok(BADGE_IDS.includes("ci"));
    assert.ok(BADGE_IDS.includes("openssf"));
  });

  it("renders codecov badge", () => {
    const badge = renderBadge({ owner: "acme", repo: "demo" }, "codecov");
    assert.match(badge.src, /codecov.io\/gh\/acme\/demo/);
  });

  it("renders bundle-size badge", () => {
    const badge = renderBadge({ npm: "@acme/pkg" }, "bundle-size");
    assert.match(badge.src, /bundlephobia/);
  });

  it("renders openssf badge", () => {
    const badge = renderBadge({ owner: "acme", repo: "demo" }, "openssf");
    assert.match(badge.src, /securityscorecards.dev/);
  });
});
...
describe("formatBadgeRow", () => {
  ...
  it("formats multiple html badges", () => {
    const row = formatBadgeRow(
      [
        { alt: "CI", href: "https://example.com", src: "https://example.com/ci.svg" },
        { alt: "npm", href: "https://example.com", src: "https://example.com/npm.svg" },
...
describe("parseFlags", () => {
  ...
  it("parses license flag", () => {
    const parsed = parseFlags(["license", "--license", "MIT"]);
    assert.equal(parsed.options.license, "MIT");
  });

  it("parses owner flag", () => {
    const parsed = parseFlags(["ci", "--owner", "acme"]);
    assert.equal(parsed.options.owner, "acme");
  });

  it("parses npm flag", () => {
    const parsed = parseFlags(["npm", "--npm", "@acme/pkg"]);
    assert.equal(parsed.options.npm, "@acme/pkg");
  });

  it("parses repo flag", () => {
    const parsed = parseFlags(["ci", "--repo", "demo"]);
    assert.equal(parsed.options.repo, "demo");
  });

  it("throws on missing flag value", () => {
    assert.throws(() => parseFlags(["--owner"]));
  });
});